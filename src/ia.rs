...
impl Ia{
    ...
    fn create_new_pop(&mut self){
        ...
        for individual in 0..16{
            if self.pop[individual][0]==0.0 && active_population> individual {
                active_population= individual -2;
            }
        }

        if active_population==1{
            println!("The best parameter are : {:?}",self.pop[1]);
        }else{
            for i in 0..5{
